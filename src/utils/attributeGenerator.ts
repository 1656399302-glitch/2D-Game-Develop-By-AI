...
// Extended AttributeTag type with auto-generated tags
export type AutoGeneratedTag = 
  | AttributeTag 
  | 'core-source' 
  | 'elemental' 
  | 'advanced-tech' 
...
// Elemental module types for auto-generation
const ELEMENTAL_MODULES: ModuleType[] = ['fire-crystal', 'lightning-conductor', 'amplifier-crystal'];

// Rare module types for advanced-tech auto-generation
const RARE_MODULES: ModuleType[] = ['temporal-distorter', 'ether-infusion-chamber', 'arcane-matrix-grid'];
...
/**
 * Auto-Generation Rules for Machine Tags (ROUND 81 PHASE 2 - D4)
 * 
 * These rules add specific tags based on machine composition:
 * 1. Has `core-furnace` → tag: `core-source`
 * 2. Has elemental module (fire-crystal, lightning-conductor, amplifier-crystal) → tag: `elemental`
 * 3. Has rare module (temporal-distorter, ether-infusion-chamber, arcane-matrix-grid) → tag: `advanced-tech`
 * 4. Has ≥5 connections → tag: `highly-connected`
 * 5. Connection density > 2.5 → tag: `dense-circuit`
 * 6. Has `shield-shell` → tag: `protective` (already in module tags)
 * 7. Has `rune-node` OR `phase-modulator` → tag: `arcane-enhanced`
 */
function applyAutoGenerationRules(
  modules: PlacedModule[],
  _connections: Connection[]
): Set<AutoGeneratedTag> {
  const autoTags = new Set<AutoGeneratedTag>();
  const moduleTypes = modules.map((m) => m.type);
  
  // Rule 1: Has `core-furnace` → tag: `core-source`
  if (moduleTypes.includes('core-furnace')) {
    autoTags.add('core-source');
  }
  
  // Rule 2: Has elemental module → tag: `elemental`
  if (moduleTypes.some((type) => ELEMENTAL_MODULES.includes(type))) {
    autoTags.add('elemental');
  }
  
  // Rule 3: Has rare module → tag: `advanced-tech`
  if (moduleTypes.some((type) => RARE_MODULES.includes(type))) {
    autoTags.add('advanced-tech');
  }
  
  // Rule 4: Has ≥5 connections → tag: `highly-connected`
  // Note: This rule checks connections count which is passed but we check it below via density calculation
  // The actual connection count check is in the caller
  
  // Rule 5: Connection density > 2.5 → tag: `dense-circuit`
  // The density check is done in getAutoGeneratedTags
  
  // Rule 6: Has `shield-shell` → tag: `protective` (already handled by MODULE_TAG_MAP)
  // This is a no-op as shield-shell already adds 'protective' tag
  
  // Rule 7: Has `rune-node` OR `phase-modulator` → tag: `arcane-enhanced`
  if (moduleTypes.includes('rune-node') || moduleTypes.includes('phase-modulator')) {
    autoTags.add('arcane-enhanced');
  }
...
/**
 * Get auto-generated tags only (ROUND 81 PHASE 2 - D4)
 */
export function getAutoGeneratedTags(
  modules: PlacedModule[],
  connections: Connection[]
): AutoGeneratedTag[] {
  const autoTags = applyAutoGenerationRules(modules, connections);
  
  // Rule 4: Has ≥5 connections → tag: `highly-connected`
  if (connections.length >= 5) {
    autoTags.add('highly-connected');
  }
  
  // Rule 5: Connection density > 2.5 → tag: `dense-circuit`
  const connectionDensity = modules.length > 0 ? connections.length / modules.length : 0;
  if (connectionDensity > 2.5) {
    autoTags.add('dense-circuit');
  }
