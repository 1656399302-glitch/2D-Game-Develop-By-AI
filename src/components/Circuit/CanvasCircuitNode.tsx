...
const ANDGateShape: React.FC<{ signalColor: string; width: number; height: number }> = ({ signalColor, width, height }) => (
  <svg width={width} height={height} viewBox="0 0 80 50" className="gate-shape">
    ...
    <circle cx="0" cy="18" r="4" fill={signalColor} className="port-input" />
    <circle cx="0" cy="32" r="4" fill={signalColor} className="port-input" />
    <circle cx="80" cy="25" r="4" fill={signalColor} className="port-output" />
  </svg>
);

const ORGateShape: React.FC<{ signalColor: string; width: number; height: number }> = ({ signalColor, width, height }) => (
  <svg width={width} height={height} viewBox="0 0 80 50" className="gate-shape">
...
const NOTGateShape: React.FC<{ signalColor: string; width: number; height: number }> = ({ signalColor, width, height }) => (
  <svg width={width} height={height} viewBox="0 0 60 40" className="gate-shape">
    ...
    <circle cx="0" cy="20" r="4" fill={signalColor} className="port-input" />
    <circle cx="60" cy="20" r="4" fill={signalColor} className="port-output" />
  </svg>
);

const NANDGateShape: React.FC<{ signalColor: string; width: number; height: number }> = ({ signalColor, width, height }) => (
  <svg width={width} height={height} viewBox="0 0 80 50" className="gate-shape">
    <path d="M 5 5 Q 5 5 20 5 L 45 5 Q 60 5 75 15 Q 75 30 75 40 Q 60 45 45 45 L 20 45 Q 5 45 5 40 Q 5 5 5 5 Z" fill="rgba(15, 23, 42, 0.9)" stroke={signalColor} strokeWidth="2" />
...
const NORGateShape: React.FC<{ signalColor: string; width: number; height: number }> = ({ signalColor, width, height }) => (
  <svg width={width} height={height} viewBox="0 0 80 50" className="gate-shape">
    <path d="M 15 5 Q 25 25 15 45 Q 35 45 55 45 L 70 45 Q 75 25 70 5 Q 55 5 35 5 Q 15 5 15 5 Z" fill="rgba(15, 23, 42, 0.9)" stroke={signalColor} strokeWidth="2" />
    <circle cx="78" cy="25" r="5" fill="none" stroke={signalColor} strokeWidth="2" />
    <circle cx="0" cy="18" r="4" fill={signalColor} className="port-input" />
...
const XORGateShape: React.FC<{ signalColor: string; width: number; height: number }> = ({ signalColor, width, height }) => (
  <svg width={width} height={height} viewBox="0 0 80 50" className="gate-shape">
    <path d="M 15 5 Q 25 25 15 45 Q 35 45 55 45 L 70 45 Q 75 25 70 5 Q 55 5 35 5 Q 15 5 15 5 Z" fill="rgba(15, 23, 42, 0.9)" stroke={signalColor} strokeWidth="2" />
...
const XNORGateShape: React.FC<{ signalColor: string; width: number; height: number }> = ({ signalColor, width, height }) => (
  <svg width={width} height={height} viewBox="0 0 80 50" className="gate-shape">
    <path d="M 15 5 Q 25 25 15 45 Q 35 45 55 45 L 70 45 Q 75 25 70 5 Q 55 5 35 5 Q 15 5 15 5 Z" fill="rgba(15, 23, 42, 0.9)" stroke={signalColor} strokeWidth="2" />
    <path d="M 10 5 Q 18 25 10 45" fill="none" stroke={signalColor} strokeWidth="2" />
    <circle cx="78" cy="25" r="5" fill="none" stroke={signalColor} strokeWidth="2" />
...
const gateShapes: Record<GateType, React.FC<{ signalColor: string; width: number; height: number }>> = {
  AND: ANDGateShape,
  OR: ORGateShape,
  NOT: NOTGateShape,
  NAND: NANDGateShape,
...
const InputNodeCanvas: React.FC<{ node: PlacedInputNode; isSelected: boolean; cycleWarning: boolean; onClick: () => void; onToggle: () => void }> = ({ node, isSelected, cycleWarning, onClick, onToggle }) => {
  ...
  return (
    <g className="circuit-node input-node" onClick={onClick} style={{ cursor: 'pointer' }} data-node-type="input" data-node-id={node.id} data-state={node.state ? 'HIGH' : 'LOW'}>
      ...
      <circle cx={size.width} cy={size.height / 2} r="5" fill={signalColor} className="port-output" data-port-index="0" />
...
const OutputNodeCanvas: React.FC<{ node: PlacedOutputNode; isSelected: boolean; cycleWarning: boolean; onClick: () => void }> = ({ node, isSelected, cycleWarning, onClick }) => {
  ...
  return (
    <g className="circuit-node output-node" onClick={onClick} style={{ cursor: 'pointer' }} data-node-type="output" data-node-id={node.id} data-signal={node.inputSignal ? 'HIGH' : 'LOW'}>
      ...
      <circle cx="0" cy={size.height / 2} r="5" fill={signalColor} className="port-input" data-port-index="0" />
...
const GateNodeCanvas: React.FC<{ node: PlacedGateNode; isSelected: boolean; cycleWarning: boolean; onClick: () => void }> = ({ node, isSelected, cycleWarning, onClick }) => {
  ...
  const borderColor = cycleWarning ? '#ef4444' : isSelected ? SIGNAL_COLORS.SELECTED : signalColor;
  const GateShape = node.gateType ? gateShapes[node.gateType] : null;
  ...
  return (
    <g className="circuit-node gate-node" onClick={onClick} style={{ cursor: 'pointer' }} data-node-type="gate" data-node-id={node.id} data-gate-type={node.gateType} data-output={node.output ? 'HIGH' : 'LOW'}>
      ...
      {GateShape && <GateShape signalColor={signalColor} width={size.width} height={size.height} />}
...
export const CanvasCircuitNode: React.FC<CanvasCircuitNodeProps> = ({ node, isSelected = false, onClick, onDragStart, onInputToggle }) => {
  ...
  const renderNode = () => {
    switch (node.type) {
      case 'input': return <InputNodeCanvas node={node as PlacedInputNode} isSelected={isSelected} cycleWarning={node.cycleWarning || false} onClick={handleClick} onToggle={handleToggle} />;
      case 'output': return <OutputNodeCanvas node={node as PlacedOutputNode} isSelected={isSelected} cycleWarning={node.cycleWarning || false} onClick={handleClick} />;
      case 'gate': return <GateNodeCanvas node={node as PlacedGateNode} isSelected={isSelected} cycleWarning={node.cycleWarning || false} onClick={handleClick} />;
      default: return null;
    }
  };
