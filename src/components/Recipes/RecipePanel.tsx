/**
 * Recipe Panel Component
 * 
 * A dedicated panel component (sidebar-style, not modal) for viewing
 * recipes with filtering, sorting, and statistics display.
 * 
 * ROUND 182: New component created for enhanced recipe viewing experience
 * Mirrors AchievementPanel pattern from Round 181
 * 
 * Features:
 * - Tab-based navigation (All, Unlocked, Locked)
 * - Category filter dropdown
 * - Sort dropdown (Recent, Name, Rarity, Unlocked)
 * - Statistics section at top
 * - Recipe list below statistics
 */

import React, { useState, useMemo, useCallback } from 'react';
import type { Recipe } from '../../data/recipes';
import { RecipeCard } from './RecipeCard';
import { RecipeStats, getRecipeCategory, getAllRecipeCategories, type RecipeCategory } from './RecipeStats';

type TabType = 'all' | 'unlocked' | 'locked';
type SortType = 'recent' | 'name' | 'rarity' | 'unlocked';

interface RecipePanelProps {
  recipes: Recipe[];
  isUnlockedMap: Record<string, boolean>;
  unlockedAtMap: Record<string, number | undefined>;
  onClose?: () => void;
}

/**
 * Compare two strings for sorting (supports Chinese characters)
 */
function compareStrings(a: string, b: string): number {
  const len = Math.min(a.length, b.length);
  
  for (let i = 0; i < len; i++) {
    const codeA = a.charCodeAt(i);
    const codeB = b.charCodeAt(i);
    if (codeA !== codeB) {
      return codeA - codeB;
    }
  }
  
  return a.length - b.length;
}

export const RecipePanel: React.FC<RecipePanelProps> = ({
  recipes,
  isUnlockedMap,
  unlockedAtMap,
  onClose,
}) => {
  // Tab state: 'all', 'unlocked', 'locked'
  const [activeTab, setActiveTab] = useState<TabType>('all');
  
  // Category filter state
  const [categoryFilter, setCategoryFilter] = useState<RecipeCategory | null>(null);
  
  // Sort state
  const [sortBy, setSortBy] = useState<SortType>('recent');

  // Add isUnlocked to recipe objects for RecipeStats component
  const recipesWithUnlockStatus = useMemo(() => {
    return recipes.map(recipe => ({
      ...recipe,
      isUnlocked: isUnlockedMap[recipe.id] ?? false,
    }));
  }, [recipes, isUnlockedMap]);

  // Filter recipes based on tab and category
  const filteredRecipes = useMemo(() => {
    let filtered = [...recipesWithUnlockStatus];
    
    // Apply tab filter
    if (activeTab === 'unlocked') {
      filtered = filtered.filter((r) => r.isUnlocked);
    } else if (activeTab === 'locked') {
      filtered = filtered.filter((r) => !r.isUnlocked);
    }
    
    // Apply category filter
    if (categoryFilter) {
      filtered = filtered.filter((r) => getRecipeCategory(r) === categoryFilter);
    }
    
    return filtered;
  }, [recipesWithUnlockStatus, activeTab, categoryFilter]);

  // Sort recipes based on sort type
  const sortedRecipes = useMemo(() => {
    const sorted = [...filteredRecipes];
    
    switch (sortBy) {
      case 'recent':
        // Unlocked recipes first, sorted by unlockedAt descending
        // Locked recipes with no unlockedAt go to the end
        sorted.sort((a, b) => {
          const aUnlocked = isUnlockedMap[a.id];
          const bUnlocked = isUnlockedMap[b.id];
          
          if (aUnlocked && !bUnlocked) return -1;
          if (!aUnlocked && bUnlocked) return 1;
          
          if (aUnlocked && bUnlocked) {
            // Both unlocked - sort by timestamp descending (newest first)
            const aTime = unlockedAtMap[a.id] ?? 0;
            const bTime = unlockedAtMap[b.id] ?? 0;
            return bTime - aTime;
          }
          return 0;
        });
        break;
        
      case 'name':
        // Sort alphabetically by name
        sorted.sort((a, b) => compareStrings(a.name, b.name));
        break;
        
      case 'rarity':
        // Sort by rarity (legendary > epic > rare > uncommon > common)
        const rarityOrder: Record<string, number> = { 
          legendary: 4, 
          epic: 3, 
          rare: 2, 
          uncommon: 1, 
          common: 0 
        };
        sorted.sort((a, b) => {
          const aOrder = rarityOrder[a.rarity] ?? 0;
          const bOrder = rarityOrder[b.rarity] ?? 0;
          return bOrder - aOrder;
        });
        break;
        
      case 'unlocked':
        // Group unlocked recipes before locked (within each group, sort by name)
        sorted.sort((a, b) => {
          const aUnlocked = isUnlockedMap[a.id];
          const bUnlocked = isUnlockedMap[b.id];
          
          if (aUnlocked && !bUnlocked) return -1;
          if (!aUnlocked && bUnlocked) return 1;
          
          // Within same unlock state, sort by name
          return compareStrings(a.name, b.name);
        });
        break;
    }
    
    return sorted;
  }, [filteredRecipes, sortBy, isUnlockedMap, unlockedAtMap]);

  // Tab click handler
  const handleTabClick = useCallback((tab: TabType) => {
    setActiveTab(tab);
  }, []);

  // Category filter change handler
  const handleCategoryChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    const value = e.target.value;
    if (value === 'all') {
      setCategoryFilter(null);
    } else {
      setCategoryFilter(value as RecipeCategory);
    }
  }, []);

  // Sort change handler
  const handleSortChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    setSortBy(e.target.value as SortType);
  }, []);

  // Get all categories for filter dropdown
  const categories = getAllRecipeCategories();
  const categoryDisplayNames: Record<RecipeCategory, string> = {
    modules: '模块',
    challenges: '挑战',
    achievements: '成就',
    tech: '科技',
  };

  return (
    <div
      className="
        fixed inset-y-0 right-0 z-[1050]
        w-full max-w-md
        bg-gradient-to-br from-[#1a1a2e] via-[#121826] to-[#0a0e17]
        border-l border-[#a855f7]/30
        shadow-2xl shadow-purple-900/20
        flex flex-col
      "
      data-testid="recipe-panel"
    >
      {/* Header */}
      <div className="p-4 pb-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[#a855f7]/20 flex items-center justify-center text-xl">
              📜
            </div>
            <div>
              <h2 className="text-xl font-bold text-white">配方</h2>
              <p className="text-sm text-[#9ca3af]">
                配方面板
              </p>
            </div>
          </div>
          
          {onClose && (
            <button
              onClick={onClose}
              className="p-2 rounded-lg hover:bg-white/10 transition-colors text-[#6b7280] hover:text-white"
              aria-label="关闭"
              data-close-panel
            >
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M5 5l10 10M15 5l-10 10" />
              </svg>
            </button>
          )}
        </div>

        {/* Tab navigation */}
        <div className="flex gap-1 mb-4">
          <button
            onClick={() => handleTabClick('all')}
            className={`
              flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all
              ${activeTab === 'all'
                ? 'bg-[#a855f7]/20 text-[#a855f7] border border-[#a855f7]/40'
                : 'bg-[#0a0e17] text-[#6b7280] border border-[#1e2a42] hover:bg-[#1e2a42]'
              }
            `}
            data-testid="recipe-tab-all"
          >
            全部
          </button>
          <button
            onClick={() => handleTabClick('unlocked')}
            className={`
              flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all
              ${activeTab === 'unlocked'
                ? 'bg-[#a855f7]/20 text-[#a855f7] border border-[#a855f7]/40'
                : 'bg-[#0a0e17] text-[#6b7280] border border-[#1e2a42] hover:bg-[#1e2a42]'
              }
            `}
            data-testid="recipe-tab-unlocked"
          >
            已解锁
          </button>
          <button
            onClick={() => handleTabClick('locked')}
            className={`
              flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all
              ${activeTab === 'locked'
                ? 'bg-[#a855f7]/20 text-[#a855f7] border border-[#a855f7]/40'
                : 'bg-[#0a0e17] text-[#6b7280] border border-[#1e2a42] hover:bg-[#1e2a42]'
              }
            `}
            data-testid="recipe-tab-locked"
          >
            未解锁
          </button>
        </div>

        {/* Filters row */}
        <div className="flex gap-2 mb-4">
          {/* Category filter */}
          <select
            value={categoryFilter ?? 'all'}
            onChange={handleCategoryChange}
            className="
              flex-1 px-3 py-2 rounded-lg text-sm
              bg-[#0a0e17] text-[#9ca3af]
              border border-[#1e2a42]
              focus:outline-none focus:border-[#a855f7]/40
              cursor-pointer
            "
            data-testid="recipe-category-filter"
          >
            <option value="all">全部分类</option>
            {categories.map((cat) => (
              <option key={cat} value={cat}>
                {categoryDisplayNames[cat]}
              </option>
            ))}
          </select>

          {/* Sort select */}
          <select
            value={sortBy}
            onChange={handleSortChange}
            className="
              flex-1 px-3 py-2 rounded-lg text-sm
              bg-[#0a0e17] text-[#9ca3af]
              border border-[#1e2a42]
              focus:outline-none focus:border-[#a855f7]/40
              cursor-pointer
            "
            data-testid="recipe-sort-select"
          >
            <option value="recent">最近解锁</option>
            <option value="name">按名称</option>
            <option value="rarity">按稀有度</option>
            <option value="unlocked">按解锁状态</option>
          </select>
        </div>
      </div>

      {/* Statistics section */}
      <div className="px-4 pb-4">
        <RecipeStats recipes={recipesWithUnlockStatus} showCategoryBreakdown={true} />
      </div>

      {/* Recipe list */}
      <div className="flex-1 overflow-y-auto px-4 pb-4" data-testid="recipe-list">
        {sortedRecipes.length > 0 ? (
          <div className="grid grid-cols-1 gap-3">
            {sortedRecipes.map((recipe) => (
              <RecipeCard
                key={recipe.id}
                recipe={recipe}
                isUnlocked={isUnlockedMap[recipe.id] ?? false}
                onClick={() => {}}
                showHint={true}
              />
            ))}
          </div>
        ) : (
          <div 
            className="flex flex-col items-center justify-center py-12 text-center"
            data-testid="recipe-empty-state"
          >
            <div className="w-16 h-16 rounded-full bg-[#1e2a42] flex items-center justify-center mb-4">
              <svg 
                width="32" 
                height="32" 
                viewBox="0 0 24 24" 
                fill="none" 
                stroke="#4a5568" 
                strokeWidth="2"
              >
                <circle cx="12" cy="12" r="10" />
                <path d="M8 12h8" />
              </svg>
            </div>
            <p className="text-[#6b7280] text-sm">
              {activeTab === 'unlocked' 
                ? '暂无已解锁的配方' 
                : activeTab === 'locked'
                ? '暂无未解锁的配方'
                : '暂无配方'}
            </p>
          </div>
        )}
      </div>
    </div>
  );
};

export default RecipePanel;
