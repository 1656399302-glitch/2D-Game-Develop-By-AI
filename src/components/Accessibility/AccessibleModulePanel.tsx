...
const MODULE_CATALOG: ModuleInfo[] = [
  { type: 'core-furnace', name: '核心熔炉', category: 'core', description: '机器的心脏。生成并放大奥术能量。' },
  { type: 'energy-pipe', name: '能量管道', category: 'pipe', description: '在模块之间传输能量。连接的关键组件。' },
  { type: 'gear', name: '齿轮组件', category: 'gear', description: '机械组件，提供稳定性和均衡能量。' },
  { type: 'rune-node', name: '符文节点', category: 'rune', description: '引导奥术力量。增强能量吞吐量。' },
  { type: 'shield-shell', name: '护盾外壳', category: 'shield', description: '防护屏障模块。提高系统稳定性。' },
  { type: 'trigger-switch', name: '触发开关', category: 'trigger', description: '激活机制。控制能量释放时机。' },
  { type: 'output-array', name: '输出阵列', category: 'output', description: '能量回路的最终终端。投射奥术光束和共振。' },
  { type: 'amplifier-crystal', name: '增幅水晶', category: 'rune', description: '棱镜能量增幅器，1个输入2个输出。分裂并放大奥术力量。' },
  { type: 'stabilizer-core', name: '稳定核心', category: 'core', description: '谐波稳定矩阵，2个输入1个输出。平衡能量波动。' },
  { type: 'void-siphon', name: '虚空虹吸', category: 'core', description: '吸收虚空能量，1个输入2个输出。以漩涡模式向内吸收能量。' },
  { type: 'phase-modulator', name: '相位调制器', category: 'rune', description: '相位偏移矩阵，2个输入2个输出。以闪电弧引导雷电能量。' },
...
/**
 * Accessible Module Panel Component
 * 
 * Enhanced module panel with comprehensive accessibility features:
 ...
 * - role="listbox" for module list
 * - role="option" for individual modules
 * - aria-selected for selection state
 * - aria-disabled for locked modules
 * - Live region for module count
 */
export function AccessibleModulePanel() {
  ...
  // Helper function to check if a module is locked
  ...
  // Keyboard navigation for module list
  const handleListKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
    switch (e.key) {
      case 'ArrowDown':
        e.preventDefault();
        setFocusedIndex(prev => Math.min(prev + 1, MODULE_CATALOG.length - 1));
        break;
      case 'ArrowUp':
        e.preventDefault();
      ...
      case 'End':
        e.preventDefault();
        setFocusedIndex(MODULE_CATALOG.length - 1);
        break;
      case 'Enter':
      case ' ':
        e.preventDefault();
        if (focusedIndex >= 0) {
          const module = MODULE_CATALOG[focusedIndex];
          if (!isModuleLocked(module.type)) {
            handleClick(module.type, false);
  ...
  const handleClick = useCallback((moduleType: ModuleType, locked: boolean) => {
    ...
    announceToScreenReader(`已添加 ${moduleType} 模块`, 'assertive');
  }, [addModule, viewport]);
  ...
  const handleRandomForge = useCallback(() => {
    ...
    saveToHistory();
    const message = `✨ ${attributes.name} 锻造成功!`;
  ...
  const handleMouseLeave = () => setHoveredModule(null);

  const unlockedCount = MODULE_CATALOG.filter(m => !isModuleLocked(m.type)).length;
    ...
    >
      ...
      <div className="p-3 border-b border-[#1e2a42] bg-gradient-to-r from-[#1a1a2e] to-[#121826]">
        <button
          ...
          className="w-full px-4 py-3 rounded-lg font-bold text-sm bg-gradient-to-r from-[#7c3aed] to-[#6d28d9] text-white hover:from-[#8b5cf6] hover:to-[#7c3aed] border border-[#a78bfa]/50 shadow-lg shadow-purple-900/30 transition-all duration-200 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] animate-pulse-subtle"
          title="随机生成2-6个模块的机器"
      ...
      <div
        ...
        role="listbox"
        aria-label="可用模块列表"
        aria-describedby="module-panel-instructions"
        onKeyDown={handleListKeyDown}
        tabIndex={0}
      >
        <div id="module-panel-instructions" className="sr-only">
          使用方向键上下导航，Enter或空格选择模块
        </div>
          ...
          {MODULE_CATALOG.map((module, index) => {
            ...
            return (
              <div
                ...
                aria-label={`${module.name}${locked ? ' (已锁定)' : ''}，${CATEGORY_NAMES[module.category]}类别`}
                tabIndex={focusedIndex === -1 ? (index === 0 ? 0 : -1) : (isFocused ? 0 : -1)}
                className={`
                  module-item arcane-card group relative transition-all duration-200 outline-none
                  ${locked ? 'cursor-not-allowed opacity-50 grayscale' : 'cursor-grab active:cursor-grabbing hover:scale-[1.02] hover:shadow-lg'}
                  ${isFocused && !locked ? 'ring-2 ring-[#00d4ff] ring-offset-2 ring-offset-[#121826]' : ''}
                  ${isSelected ? 'bg-[#00d4ff]/10 border-[#00d4ff]/50' : ''}
                `}
                style={{
                  borderLeftColor: CATEGORY_COLORS[module.category],
                  borderLeftWidth: '3px',
                  ...(locked && rarityStyle ? { borderColor: `${rarityStyle.primary}40` } : {}),
                }}
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`w-12 h-12 rounded flex items-center justify-center text-2xl relative ${locked ? 'bg-gray-800' : ''}`}
                    style={{
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {locked && recipe ? (
                      <>
                        <p className="text-xs text-gray-600 mt-1 italic">{recipe.hint}</p>
                      ...
                      <>
                        <p className="text-xs text-[#9ca3af] mt-1 line-clamp-2">{module.description}</p>
                        <div className="flex items-center gap-2 mt-2">
                          <span
                            className="text-xs px-2 py-0.5 rounded"
                            style={{
                ...
                {locked && (
                  <div className="absolute inset-0 rounded-lg pointer-events-none" style={{ background: 'linear-gradient(135deg, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.1) 100%)' }} />
                )}
      ...
      {hoveredModule && !isModuleLocked(hoveredModule.type) && (
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <span className="w-3 h-3 rounded-full" style={{ backgroundColor: CATEGORY_COLORS[hoveredModule.category] }} aria-hidden="true" />
            <h4 className="text-sm font-semibold text-white">{hoveredModule.name}</h4>
          </div>
          <p className="text-xs text-[#9ca3af] leading-relaxed">{hoveredModule.description}</p>
          <div className="mt-2 pt-2 border-t border-[#1e2a42]">
            <p className="text-[10px] text-[#6b7280]">💡 提示: 点击或拖拽添加模块</p>
          </div>
        </div>
      )}
      ...
      <div className="p-3 border-t border-[#1e2a42]">
        <p className="text-xs text-[#4a5568] text-center">
          共 {MODULE_CATALOG.length} 种模块 ({unlockedCount} 已解锁)
        </p>
      </div>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          .module-panel { width: 100%; max-height: 200px; border-r: none; border-bottom: 1px solid #1e2a42; }
        }
      `}</style>
    </div>
  );
...
export default AccessibleModulePanel;
